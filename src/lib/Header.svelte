...
<header class="flex flex-row justify-between h-10 dark:bg-black dark:text-white">
	<div>
		<label for="">dark</label>
		<Toggle checked={theme === 'dark'} on:click="{() => {
			if (theme === 'light') {
				theme = 'dark'
				localStorage.setItem('theme', 'dark')
			} else {
	...
	<nav>
		<ul class="flex flex-row space-x-5 text-xl font-bold">
